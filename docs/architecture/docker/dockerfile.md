...
FROM ubuntu:18.04
MAINTAINER Sanket # Deprecated use label instead
LABEL org.opencontainers.image.authors="bajoriasanket@gmail.com"
WORKDIR /app
ARG user1 # ARG only available using build stage
ARG buildno=1 # ARG with default value
ENV VER=v1.0.0 # ENV available on both build and run time, can be used anywhere as $VER
ENV buildno=2 # ENV will override ARG with same variable name
EXPOSE 8443 # For documentation tell about which port get exposed
COPY . /app # Copy from local to container
RUN make /app $user1 # $user1 will be expanded and will be passed as parameter to make instructuion
VOLUME /app # will mount /app from container to host system
...
```sh
docker run ubuntu ls /app # ls /app will become default command
```
...
Can be modified and published through

```sh
docker run --expose=443 nginx # Add to expose port list	
docker run -p 8443:443 nginx # Binding 8443 host port to 443 of container port
docker run -p 443 nginx # Bind 443 port from host and container
docker run -P nginx # it will bind all exposed port from container to ephemeral port in host
```
...
```sh
docker run -v /foo nginx # mount /foo volume anonymously
docker run -v awesome:/foo nginx # mount /foo volume with awesome name
docker run -v /host/foo:/foo nginx # mount host path /host/foo to /foo path in container.
docker run --rm -v /foo -v awesome:/bar busybox top # /foo will be removed since it has no name due to --rm (clean up after stop) but /bar won't be
```
